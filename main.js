...
async function loadTeamsData() {
    ...
    try {
        ...
        for (const doc of teamsSnapshot.docs) {
            ...
            // Obtener las respuestas del equipo
            const answersSnapshot = await db.collection('answers')
                .where('teamId', '==', doc.id)
                .get();

            const totalAnswered = team.questionsAnswered?.length || 0;
            const answeredQuestions = team.questionsAnswered || [];

            teamsHTML += `
                <div class="team-card" data-team-id="${doc.id}">
                    <h4>${team.teamName}</h4>
                    <p>Colegio: ${team.schoolName}</p>
                    <p>Preguntas Respondidas: ${totalAnswered} de 25</p>
                    <div class="progress-bar">
                        <div class="progress" style="width: ${(totalAnswered / 25) * 100}%"></div>
                    </div>
                    <div class="questions-status">
                        <h5>Estado de Preguntas:</h5>
                        <div class="questions-grid-status">
                            ${generateQuestionsStatus(answeredQuestions)}
                        </div>
                    </div>
                    <div class="team-actions">
                        <button onclick="viewTeamDetails('${doc.id}')">Ver Detalles</button>
                        <button onclick="editTeam('${doc.id}')">Editar</button>
                        <button onclick="deleteTeam('${doc.id}')">Eliminar</button>
                    </div>
                </div>
            `;
        }
...
function generateQuestionsStatus(answeredQuestions) {
    let html = '';
    for (let i = 1; i <= 25; i++) {
        const isAnswered = answeredQuestions.includes(i);
        html += `
            <div class="question-status ${isAnswered ? 'answered' : 'pending'}" 
                 title="Pregunta ${i}: ${isAnswered ? 'Respondida' : 'Pendiente'}">
                ${i}
            </div>
        `;
    }
